...
if TYPE_CHECKING:
    ...
    PacketData = Tuple[User, 'VoiceData']
...
class PacketDecoder(threading.Thread):
    ...
    def feed_rtp(self, packet: RTPPacket) -> None:
        if self._end_thread.is_set():
            # log.debug("New packets after thread end in %s", self.name)
            return
    ...
    def flush(self) -> None:
        with self._lock:
            # log.debug('Flushing %s (buffer=%s)', self.name, len(self._buffer))
            # log.debug('%s', sorted(self._buffer._buffer))
...
if False:
    ...
    #         ... # XTODO: rotating buffer of Nones or something
    ...
    #     def reset(self):
    #         with self._lock:
    #             self._decoder = Decoder() # XTODO: Add a reset function to Decoder itself
    #             self._last_seq = self._last_ts = 0
    #             self._buffer.clear()
    #             self._primed.clear()
    #             self._end_main_loop.set() # _XXX: racy with _push?
    ...
    #             time.sleep(0.001)
    #             # XTODO: Maybe only wait at most for about as long we we're supposed to?
    ...
    #         self._decoder = Decoder()
    #         self._buffer = []
    #         self._rtcp_buffer = {} # XTODO: Add RTCP queue
    #         self._last_seq = self._last_ts = 0
    #
    #         # Optional diagnostic state stuff
    #         self._overflow_mult = self._overflow_base = 2.0
